...
const searchedDevices = computed<Device[]>(() => {
    ...
    const matched = smartSearch(
        s,
        props.devices.map((d) => d.name),
        0.76
    );
    const out: Device[] = [];
...
function exportCSV(): void {
    ...
    for (const d of rowsToExport) {
        const row = [d.ip ?? "", d.name ?? "", String(getAbonsUp(d)), d.vendor ?? "", d.model ?? "", d.group ?? ""].map(
                    ...
                    <tr class="text-xs uppercase tracking-wide text-gray-600 dark:text-gray-300">
                        ...
                        <th v-if="hasInterfacesCount" class="px-4 py-3 text-left font-semibold">
                            <button
                                ...
                                @click="toggleSort('interfaces_count.abons_up')"
                            >
                                <span>Абоненты</span>
                            ...
                            <div class="group/ip flex items-center gap-3">
                                ...
                                >
                                    <svg
                                        ...
                                        height="18"
                                        fill="currentColor"
                                        class="inline group-hover/console:hidden"
                                        viewBox="0 0 16 16"
                                    >
                                        <path
                                    ...
                                    <svg
                                        ...
                                        height="18"
                                        fill="currentColor"
                                        class="hidden group-hover/console:inline"
                                        viewBox="0 0 16 16"
                                    >
                                        <path
                        ...
                        <td class="px-4 py-3">
                            <div v-if="dev.model" class="flex items-center gap-2 group">
                                ...
                                <Button
                                    ...
                                    size="small"
                                    outlined
                                    class="rounded-xl! opacity-0 group-hover:opacity-100"
                                />
                                <Button
                                    v-else
                                    @click="modelFilter = null"
                                    icon="pi pi-filter-slash"
                                    size="small"
                                    outlined
                                    class="rounded-xl!"
