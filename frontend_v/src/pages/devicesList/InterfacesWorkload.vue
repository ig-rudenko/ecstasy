    ...
    <div v-if="dev.interfaces_count && dev.interfaces_count.count">
        <div style="width: 25rem">
            <div class="flex rounded-xl text-center">
                ...
                <div
                    v-if="dev.interfaces_count.abons_up_with_desc"
                    v-tooltip="'Абонентские порты (UP) с описанием'"
                    class="bg-green-700 text-gray-950 text-xs first:rounded-l last:rounded-r"
                    :style="{ width: abonsUpWithDescPercent + '%' }"
                ...
                <div
                    class="bg-green-500 text-gray-900 text-xs first:rounded-l last:rounded-r"
                    v-if="dev.interfaces_count.abons_up_no_desc"
                    v-tooltip="'Абонентские порты (UP) без описания'"
                    :style="{ width: abonsUpNoDescPercent + '%' }"
                    :aria-valuemax="totalCount"
                >
                ...
                <div
                    class="bg-red-300 text-gray-900 text-xs first:rounded-l last:rounded-r"
                    v-if="dev.interfaces_count.abons_down_with_desc"
                    v-tooltip="'Абонентские порты (DOWN) с описанием'"
                    :style="{ width: abonsDownWithDescPercent + '%' }"
                    :aria-valuemax="totalCount"
                >
                ...
                <div
                    class="bg-gray-300 text-gray-900 text-xs first:rounded-l last:rounded-r"
                    v-if="dev.interfaces_count.abons_down_no_desc"
                    v-tooltip="'Незадействованные порты'"
                    :style="{ width: abonsDownNoDescPercent + '%' }"
                    :aria-valuemax="totalCount"
                >
                ...
                <!-- СЛУЖЕБНЫЕ порты -->
                <div
                    v-if="systeminterfaces_count"
                    v-tooltip="'Служебные порты'"
                    class="progress-bar bg-blue-400 text-gray-900 text-xs first:rounded-l last:rounded-r"
                    :style="{ width: systemInterfacesPercents + '%' }"
                    :aria-valuemax="totalCount"
                >
