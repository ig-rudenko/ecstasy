 ...
 */
const activeTasksCount = computed(() => tasks.value.filter((task) => !isTaskFinished(task)).length);
...
function createTaskEntry(
    response: BulkCommandTaskLaunch,
    command: DeviceCommandTemplate,
 ...
 */
function showHistoryResultOutput(
    entry: BulkCommandExecutionHistoryEntry,
    result: BulkCommandExecutionHistoryResult
): void {
    showDeviceOutput({
        deviceId: result.deviceId || 0,
        deviceName: result.deviceName,
        status: ["SUCCESS", "ERROR", "SKIPPED"].includes(result.status)
            ? (result.status as TaskDeviceStatus)
            : "PENDING",
...
<template>
                                ...
                                <div>
                                    ...
                                    <div class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                        {{
                                            ...
                                            "Выберите шаблон команды и заполните параметры перед запуском."
                                        }}
                                    </div>
                                </div>
                            ...
                            >
                                <template #legend="{ toggleCallback }">
                                    <div class="flex w-full flex-wrap items-center gap-3 px-2 py-1">
                                        ...
                                        <Tag
                                            :severity="isTaskFinished(task) ? 'contrast' : 'info'"
                                            :value="task.status"
                                        />
                                    </div>
                                </template>
                                ...
                                <div class="flex flex-col gap-4">
                                    ...
                                    <div
                                        v-if="getWaitingDevices(task).length"
                                        ...
                                        <table class="min-w-195 w-full text-sm">
                                            ...
                                            <tbody>
                                                ...
                                                >
                                                    <td class="px-3 py-3">
                                                        <div
                                                            class="font-medium text-gray-900 dark:text-gray-100 text-nowrap font-mono"
                                                        >
                                                            {{ device.deviceName }}
                                                        </div>
                                                    ...
                                                    <td class="px-3 py-3">
                                                        <Badge
                                                            :severity="getDeviceSeverity(device.status)"
                                                            :value="device.status"
                                                        />
                                                    </td>
                                                    <td class="px-3 py-3">
                                                        <div
                                                            class="max-w-xl truncate text-sm text-gray-600 dark:text-gray-300"
                                                        >
                    ...
                    >
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <Button
                                        icon="pi pi-list"
                                        :label="
                                            getHistoryResultsState(entry.id).loaded
                                                ? 'Обновить устройства'
                                        ...
                                        @click="
                                            loadHistoryResults(
                                                entry.id,
                                                getHistoryResultsState(entry.id).loaded
                                                    ? getHistoryResultsState(entry.id).page
                                                    : 1
                                            )
                                        "
                                    />
                                ...
                                <table class="min-w-230 w-full text-sm">
                                    ...
                                    <tbody>
                                        ...
                                        >
                                            ...
                                            <td class="px-3 py-3">
                                                <Badge
                                                    :severity="getDeviceSeverity(result.status)"
                                                    :value="result.status"
                                                />
                                            </td>
                                            <td class="px-3 py-3">
                                                <div
                                                    class="max-w-xl truncate font-mono text-xs text-gray-700 dark:text-gray-200"
                                                >
                                            ...
                                            <td class="px-3 py-3">
                                                <div class="max-w-xl truncate text-sm text-gray-600 dark:text-gray-300">
                                                    {{
                                                        ...
                                                        result.detail ||
                                                        (result.output
                                                            ? `Вывод получен за ${result.duration.toFixed(3)} c`
                                                            : "Без вывода")
                                                    }}
                                                </div>
                                            </td>
                            ...
                            <Paginator
                                v-if="
                                    getHistoryResultsState(entry.id).loaded &&
                                    getHistoryResultsState(entry.id).total > 20
                                "
                                :rows="20"
                                ...
                                @page="
                                    (event: any) => {
                                        loadHistoryResults(entry.id, event.page + 1);
                                    }
                                "
                                :pt="{
                                    root: {
    ...
    <Dialog
        ...
        modal
        maximizable
        :header="currentOutput?.deviceName || 'Вывод'"
    ...
    >
        <div v-if="currentOutput" class="flex flex-col gap-4">
            ...
            <pre
                ...
                >{{ currentOutput.output || "Вывод отсутствует" }}</pre
            >
        </div>
    </Dialog>
