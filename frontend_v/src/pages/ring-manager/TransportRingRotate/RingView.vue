<template>
    <div>
        <template v-for="(dev, index) in ringsPoints" :key="dev.name">
            <div
                v-if="index !== 0"
                :class="['port', 'bottom'].concat(portStatusClass(dev, dev.port_to_prev_dev))"
            ...
            <template v-if="index !== ringsPoints.length - 1">
                <div
                    :class="portStatusClass(dev, dev.port_to_next_dev)"
                    class="port top"
