    ...
    <div class="flex flex-col gap-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="mt-1 text-sm text-gray-600 dark:text-gray-300">
                        {{ rings.selectedRing.description }}
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <Tag
                    :severity="rotatingNow ? 'warn' : 'secondary'"
                    :value="rotatingNow ? 'Идёт разворот' : 'Ожидание'"
                />
            </div>
        </div>
            ...
            >
                ...
                >
                    ...
                    <div class="mt-2 flex flex-wrap gap-2">
                        <Badge v-for="vlan in rings.selectedRing.vlans" :key="vlan" severity="secondary">{{
                            vlan
                        }}</Badge>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async periodicalRingCheck() {
            try {
                let resp = await api.get(
                    "/api/v1/ring-manager/transport-ring/" + this.rings.selectedRing.name + "/status"
                );
                this.rotatingNow = resp.data.rotating;
                this.ringActive = resp.data.active;
            } catch (e) {
        ...
        async getLastSolutions() {
            try {
                const resp = await api.get(
                    "/api/v1/ring-manager/transport-ring/" + this.rings.selectedRing.name + "/solutions/last"
                );
        ...
        async getSolutions() {
            ...
            try {
                const resp = await api.get(
                    "/api/v1/ring-manager/transport-ring/" + this.rings.selectedRing.name + "/solutions"
                );
                this.points = resp.data.points;
                this.solutions = resp.data.solutions;
                this.safeSolutions = resp.data.safeSolutions;
