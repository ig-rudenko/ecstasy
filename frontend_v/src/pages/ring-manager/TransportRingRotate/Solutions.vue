<template>
        ...
        <template v-for="(sol, index) in solutions" :key="index">
            ...
            <div v-if="sol.hasOwnProperty('set_port_status')" class="solution-card">
                ...
                <div v-else class="shrink-0">
                    <SolutionStatus :status="sol.set_port_status.perform_status" :error="sol.set_port_status.error" />
                </div>
                ...
                <div class="flex-1">
                    ...
                    <div class="mt-3 font-semibold text-gray-900 dark:text-gray-100">
                        {{ sol.set_port_status.device.name }}
                    </div>
                    <div class="mt-2 flex flex-wrap items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span>на</span>
                        <span :class="portStatusClasses(sol.set_port_status.status)">{{
                            sol.set_port_status.status
                        }}</span>
                    </div>
                </div>
            </div>
            ...
            <div v-if="sol.hasOwnProperty('set_port_vlans')" class="solution-card">
                <div v-if="!performed" class="solution-index">{{ index + 1 }}.</div>
                <div v-else class="shrink-0">
                    <SolutionStatus :status="sol.set_port_vlans.perform_status" :error="sol.set_port_vlans.error" />
                </div>
                ...
                <div class="flex-1">
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span>VLAN</span>
                        <Badge severity="secondary" v-for="vlan in sol.set_port_vlans.vlans" :key="vlan">{{
                            vlan
                        }}</Badge>
                        <span>на порту</span>
    ...
    <Dialog v-model:visible="showModal" modal>
        <div class="text-base leading-relaxed text-gray-800 dark:text-gray-100">
            Соглашаясь, вы подтверждаете, что ознакомились с перечнем решений и он соответствует вашим требованиям. Все
            действия выполнятся автоматически.
        </div>
