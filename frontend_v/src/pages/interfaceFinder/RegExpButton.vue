...
<template>
    <Button
        type="button"
        :severity="regexpEnabled ? 'warn' : 'secondary'"
        :outlined="!regexpEnabled"
        :disabled="disabled"
        :class="btnClass"
        v-tooltip.bottom="regexpEnabled ? 'Включить обычный текст' : 'Включить Regex'"
        @click="() => emit('click')"
    >
        <template #icon>
            <svg
