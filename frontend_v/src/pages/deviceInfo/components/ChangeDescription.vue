<template>
    <div v-if="displayMode" class="flex gap-2 items-center p-2 group">
        <div
            v-if="dynamicDescription.length"
            class="font-mono max-sm:text-sm"
            v-html="dynamicDescription"
            @click="
                () => {
                    if (interface.link) $emit('changeDevice', interface.link.deviceName);
                }
            "
        ></div>
    ...
    <div v-else class="px-2">
        ...
        <Message @click="errors = ''" v-if="errors" severity="error" :pt="{ content: { class: 'py-0' } }">{{
            errors
        }}</Message>
    </div>
</template>
    ...
    computed: {
        dynamicDescription(): string {
            ...
            return this.interface.description.replace(
                ...
                (s) =>
                    `<span class="bg-yellow-200 dark:bg-yellow-900 dark:bg-opacity-70"><span class="text-dark text-decoration-none cursor-pointer">${s}</span></span>`
            );
        },
        inputClasses(): string[] {
