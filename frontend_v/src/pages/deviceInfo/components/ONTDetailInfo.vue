<template>
    <tr :style="lineStyle(line[1])" :class="lineClasses" class="hover:ring-1 ring-indigo-500">
        ...
        <td class="px-2">
            <div
                :style="statusStyles(line[1])"
                class="text-center text-gray-950 rounded-2xl px-2 py-1.5 font-mono hover:shadow-md"
            >
                {{ line[1] }}
            </div>
        </td>
    ...
    <tr v-if="showDetailInfo">
        <td colspan="7" v-if="complexInfo">
            <!--      DETAIL PORT INFO  -->
            <div v-if="complexInfo.portDetailInfo" class="p-6">
...
export default defineComponent({
    ...
    methods: {
        ...
        getDetailInfo() {
            ...
            api.get("/api/v1/devices/" + this.deviceName + "/interface-info?port=" + this.ontInterface.name).then(
