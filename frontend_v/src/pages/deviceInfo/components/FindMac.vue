<template>
    <Dialog v-model:visible="macSearch.dialogVisible" modal class="w-[min(96vw,1100px)]" content-class="!p-0">
        <template #header>
        ...
        <div class="flex flex-col gap-5 bg-gray-50/60 p-4 dark:bg-gray-950/30 sm:p-6">
            ...
            >
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div
                        v-if="macSearch.lastMac"
                        class="inline-flex w-fit items-center gap-2 rounded-full border border-indigo-200/80 bg-indigo-50 px-3 py-1.5 font-mono text-sm text-indigo-900 dark:border-indigo-900/70 dark:bg-indigo-500/15 dark:text-indigo-100"
                    >
                ...
                <div
                    v-else
                    class="mt-4 flex justify-center rounded-2xl border border-dashed border-gray-200/80 bg-white/70 p-8 dark:border-gray-700/80 dark:bg-gray-950/25"
                >
                    <ProgressSpinner />
                </div>
            </section>
            ...
            >
                <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div v-if="zabbixLinks.length" class="flex flex-wrap gap-2">
                        ...
                        >
                            <Button severity="danger" size="small" :label="zbx.name" class="rounded-full! text-xs!" />
                        </a>
                    </div>
                </div>
                ...
                <div v-if="macSearch.lastMacDetail" class="mt-4 flex flex-col gap-4">
                    ...
                    >
                        ...
                        <div class="mt-3 grid gap-3 md:grid-cols-2">
                            <router-link
                                v-for="device in ecstasyDevices"
                                :key="device.id"
                                :to="device.url"
                    ...
                    >
                        <div
                            class="flex flex-col gap-3 border-b border-gray-200/80 bg-gray-50/80 px-4 py-4 dark:border-gray-700/80 dark:bg-gray-900/70 lg:flex-row lg:items-center lg:justify-between"
                        >
                            <div class="flex min-w-0 items-center gap-3">
                                <div
                                    class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-sky-100 text-sky-700 dark:bg-sky-500/15 dark:text-sky-300"
                                >
                            ...
                            <div
                                class="inline-flex w-fit items-center gap-2 rounded-full border border-emerald-200/80 bg-emerald-50 px-3 py-1.5 text-sm text-emerald-800 dark:border-emerald-900/70 dark:bg-emerald-500/10 dark:text-emerald-200"
                            >
                        ...
                        <div class="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-3">
                            ...
                            >
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    <span
                                        class="w-full text-center rounded-full bg-indigo-600 px-2.5 py-1 text-[16px] font-mono font-semibold text-white dark:bg-indigo-400 dark:text-slate-950"
                                    >
                                        VLAN {{ res.vlan }}
                                    </span>
                                </div>
                                ...
                                <dl class="mt-4 flex flex-col gap-3">
                                    <div>
                                        ...
                                        <dd class="mt-1 font-mono text-sm text-gray-800 dark:text-gray-100">
                                            {{ res.ip }}
                                        </dd>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <dd class="mt-1 font-mono text-sm text-gray-800 dark:text-gray-100 break-all">
                                            {{ res.mac }}
                                        </dd>
                                    </div>
                                    ...
                                    <div v-if="res.port">
                                        ...
                                        <dd class="mt-1 font-mono text-sm text-gray-700 dark:text-gray-200">
                                            {{ res.port }}
                                        </dd>
                                    </div>
                                </dl>
...
export default defineComponent({
    ...
    computed: {
        ...
        ecstasyDevices() {
            return this.macSearch.lastMacDetail?.ecstasy_devices || [];
        },
    },
    data() {
        return {
