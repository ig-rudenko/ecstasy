    ...
    <tr :class="lineClasses">
        ...
        <td class="px-2">
            <div
                ...
                class="dark:text-gray-950 dark:opacity-70 rounded-2xl px-4 py-1.5 font-mono hover:shadow-md"
            >
                {{ line[1] }}
            </div>
        </td>
...
const subscriberSummary = computed<OltSubscriberSummary>(
    () =>
        props.subscriberRow || {
            customers: [],
            addresses: [],
...
function getDetailInfo() {
    ...
    api.get(`/api/v1/devices/${props.deviceName}/interface-info?port=${ontInterface.value.name}`).then(
