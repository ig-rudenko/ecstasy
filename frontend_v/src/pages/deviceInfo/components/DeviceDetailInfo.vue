<template>
    <div>
        <Button
            v-tooltip.right="'Подробная информация'"
            outlined
            @click="visible = true"
            class="rounded-2xl not-sm:px-2 shadow-sm border-none"
        >
            <svg
    ...
    <Drawer
        ...
        class="w-full md:w-80! lg:w-140!"
        position="right"
        :modal="false"
        :dismissable="false"
    >
        <template #header>
            <div class="text-xl sm:text-2xl">Детальная информация</div>
        </template>
        <div v-if="generalInfo.zabbixInfo" class="flex flex-col gap-3">
            <div v-if="generalInfo" class="font-mono text-xs sm:text-sm py-1 space-y-1">
            ...
            <div v-if="generalInfo.zabbixInfo.description" class="p-3 border rounded-xl text-xs sm:text-sm">
                {{ generalInfo.zabbixInfo.description }}
            </div>

            <div class="grid grid-cols-3 gap-4 font-mono text-xs sm:text-sm wrap-break-word">
    ...
    methods: {
        async getImages() {
            ...
            for (let i = 1; i < 10; i++) {
                const url =
                    "/img/devices/" +
                    this.generalInfo.vendor.toLowerCase() +
                    "/" +
                    this.generalInfo.model.toUpperCase().replace("/", "-") +
                    "_" +
                    i +
                    ".png";
                try {
