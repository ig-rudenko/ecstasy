<template>
    <Dialog maximizable modal v-model:visible="brasSessionsService.dialogVisible" class="w-full h-full">
        ...
        <div v-if="canCutBrasSession">
            ...
            <!--        НОЖ-->
            <SplitButton
                v-if="brasSessionsService.current?.device && brasSessionsService.current?.port"
                outlined
                rounded
    ...
    <svg xmlns="http://www.w3.org/2000/svg" style="display: none">
        <symbol id="blade-handmade">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512.001 512.001" xml:space="preserve">
                <polygon
                    style="fill: #4eb9ff"
                    points="206.999,341.441 142.29,276.733 88.251,330.773 89.293,394.438 119.489,428.951 "
                ></polygon>
                <polygon
                    style="fill: #9ad7ff"
                    points="88.251,330.773 22.548,396.476 53.786,494.653 119.489,428.951 "
                ></polygon>
                <path
                    style="fill: #174461"
                    d="M459.688,112.504c24.428-24.428,24.428-64.034,0-88.462c-24.428-24.428-64.034-24.428-88.462,0  l-9.829,9.829l27.85,60.613l60.613,27.85L459.688,112.504z"
                ></path>
                <polygon
                    style="fill: #ff755c"
                    points="361.397,33.872 114.032,281.237 135.328,348.403 202.493,369.7 449.86,122.334 "
                ></polygon>
                <path
                    style="fill: #9ad7ff"
                    d="M376.288,135.293c7.69-7.69,7.69-20.159,0-27.85s-20.159-7.69-27.85,0l-42.593,42.593l-7.098,25.665  l-25.665,7.099l-91.738,91.738c-7.69,7.69-7.69,20.159,0,27.85s20.159,7.69,27.85,0l91.738-91.738l7.098-25.665l25.665-7.098  L376.288,135.293z"
                ></path>
                <rect
                    x="280.221"
                    y="160.657"
                    transform="matrix(-0.7071 0.7071 -0.7071 -0.7071 645.442 93.347)"
                    style="fill: #174461"
                    width="46.334"
                    height="39.384"
                ></rect>
...
export default defineComponent({
    ...
    methods: {
        formatSession(input: string): string {
            // Заменяем Domain
            input = input.replace(/(Domain name\s*:\s*)(\S+)/, (_, prefix, value) => {
                return `${prefix}<span class="px-2 py-1 rounded bg-indigo-500 text-white">${value}</span>`;
            });
            // Заменяем MAC-адрес
            input = input.replace(/(User MAC\s*:\s*)(\S+)/, (_, prefix, value) => {
            ...
            for (let regex of regexes) {
                ...
                input = input.replace(regex, (_, prefix, value) => {
            ...
            for (let line of lines) {
                ...
                if (insideActiveBlock && line.includes("Service info")) {
                    const replacedLine = line.replace(
                        /(Service info\s*:\s*)(.+)/,
                        (_, prefix, value) =>
                            `${prefix}<span class="px-2 py-1 rounded bg-violet-300 dark:bg-violet-800">${value.trim()}</span>`
                    );
