<template>
    ...
    <Dialog modal v-model:visible="interfaceControlService.dialogVisible" header="Внимание">
        <template #header>
            ...
            <div class="text-center text-2xl" style="padding-left: 10px">Внимание</div>
        </template>
        <div>
            <div
                class="flex flex-wrap gap-3 py-6 items-center justify-center text-2xl"
                v-if="interfaceControlService.portAction.action"
            >
                ...
                <svg class="scale-125" width="24" height="24">
                    ...
                    <use
                        v-else-if="interfaceControlService.portAction.name === 'выключить'"
                        xlink:href="#port-down-icon"
                    ></use>
                    <use
                        v-else-if="interfaceControlService.portAction.name === 'перезагрузить'"
                        xlink:href="#port-reload-icon"
                    ></use>
                </svg>
            ...
            <div v-else class="text-center textl-xl">Неверное действие</div>
            ...
            <div class="flex flex-wrap gap-2 justify-end">
                ...
                <Button
                    icon="pi pi-check"
                    v-show="interfaceControlService.portAction.action"
                    @click="() => submitPortAction(false)"
                    class="rounded-2xl"
                ...
                <Button
                    icon="pi pi-check"
                    severity="success"
                    label="Сохранить конфигурацию после"
                    v-show="interfaceControlService.portAction.action"
                    class="rounded-2xl"
