<template>
  <div class="flex w-full flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
    <div class="flex flex-wrap items-center gap-4">
      <span v-if="status === 0" class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-300">
      ...
      <span v-else-if="status !== 1" class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gray-100 text-gray-500 dark:bg-gray-500/15 dark:text-gray-300">
        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" fill="currentColor" class="bi bi-question-circle" viewBox="0 0 16 16">
          <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
          <path d="M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
        </svg>
      </span>
      <span v-else class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-300">
      ...
      <button type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-gray-200/70 bg-white/80 text-gray-500 transition hover:text-indigo-600 dark:border-gray-700/70 dark:bg-gray-950/40 dark:text-gray-300 dark:hover:text-indigo-300" @click="toggleStatus">
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <div v-show="device.ip" class="inline-flex items-center rounded-2xl bg-indigo-600 px-3.5 py-1.5 font-mono text-sm text-white sm:text-base">
            {{ device.ip }}
          </div>
          <div class="relative">
            <button type="button" class="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-gray-200/70 bg-white/80 text-gray-500 transition hover:text-indigo-600 dark:border-gray-700/70 dark:bg-gray-950/40 dark:text-gray-300 dark:hover:text-indigo-300" @click="copyIP">
              <i class="pi pi-clone"/>
            </button>
            <span v-if="copied" class="absolute left-full top-1/2 z-10 ml-2 -translate-y-1/2 whitespace-nowrap rounded-xl bg-white px-3 py-2 text-xs shadow dark:bg-surface-800">
