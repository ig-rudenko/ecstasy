...
import { DeviceCommandTemplate, executeDeviceCommand, validateDeviceCommand } from "@/services/deviceCommands";
            ...
            >
                <div class="mb-4 flex items-center justify-between gap-3">
                    <div>
                        ...
                        <div class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            История выполнения в рамках текущего диалога.
                        </div>
                    </div>
                </div>
                ...
                <div v-if="results.length" class="flex flex-col gap-3 font-mono">
                    ...
                    >
                        <template #legend="{ toggleCallback }">
                            <div class="flex w-full items-center gap-3 px-2 py-1">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="whitespace-pre-wrap break-all text-xs text-gray-700 dark:text-gray-200">
                                        {{ data.command }}
                                    </div>
                                </div>
                                <Button
                        ...
                        <pre
                            v-else-if="data.output"
                            class="overflow-auto whitespace-pre-wrap rounded-2xl bg-gray-950 px-4 py-3 text-xs text-gray-100"
                            >{{ data.output }}</pre
                        >
