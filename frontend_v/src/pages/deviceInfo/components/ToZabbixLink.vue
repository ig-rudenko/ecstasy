<template>
  <Button text :class="blockClasses" severity="danger"
          v-tooltip.bottom="monitoringAvailable?'В Zabbix':'Снято с мониторинга'">
    <a v-if="zabbixHostId" :href="zabbixUrl + '/hostinventories.php?hostid=' + zabbixHostId" class="items-center flex"
       target="_blank">
...
export default defineComponent({
  props: {
    zabbixHostId: {required: true, type: Number},
    zabbixUrl: {required: true, type: String},
    monitoringAvailable: {required: true, type: Boolean},
  },
  computed: {
    blockClasses() {
      if (this.monitoringAvailable) return []
      return ["border-1", "border-red-500", "rounded"]
    }
  }
})
</script>