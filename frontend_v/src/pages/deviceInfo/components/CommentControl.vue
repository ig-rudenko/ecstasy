<template>
    ...
    <Dialog modal v-model:visible="commentService.dialogVisible">
        ...
        <div>
            <div
                v-if="commentService.commentObject.action === 'add' || commentService.commentObject.action === 'update'"
            >
                <Textarea
                    class="w-full p-3"
                    autoResize
                    autofocus
                    v-model="commentService.commentObject.text"
                    style="height: 170px"
                ></Textarea>
            </div>
            ...
            <div v-else-if="commentService.commentObject.action === 'delete'" class="text-center">
                ...
                <div class="whitespace-break-spaces text-left p-3 border rounded-xl m-3">
                    {{ commentService.commentObject.text }}
                </div>
            </div>
        ...
        <div class="flex justify-end gap-2">
            <Button
                label="Нет"
                autofocus
                @click="commentService.dialogVisible = false"
                icon="pi pi-times"
                severity="secondary"
                class="rounded-2xl"
            />
            ...
            <Button
                v-if="commentService.commentObject.action === 'delete'"
                icon="pi pi-trash"
                label="Да"
                severity="danger"
                class="rounded-2xl"
