<template>
    <div class="flex flex-col gap-5">
        <div
            class="flex flex-col gap-4 rounded-3xl border border-gray-200/80 bg-white/80 p-4 dark:border-gray-700/80 dark:bg-gray-900/55"
        >
            <div class="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <div>
                    <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">Сравнение конфигураций</div>
                    <div class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Выберите две версии и сравните изменения построчно.
                    </div>
                </div>

                <div class="flex flex-wrap items-center gap-3">
                    <div
                        class="inline-flex items-center gap-3 rounded-2xl border border-gray-200/80 bg-gray-50/80 px-3 py-2 text-sm text-gray-700 dark:border-gray-700/80 dark:bg-gray-800/60 dark:text-gray-300"
                    >
        ...
        <div
            v-if="compareError"
            class="rounded-3xl border border-red-200/80 bg-red-50/70 px-4 py-3 text-sm text-red-700 dark:border-red-900/70 dark:bg-red-950/20 dark:text-red-200"
        >
            {{ compareError }}
        </div>
        ...
        <div
            v-if="rows.length"
            class="overflow-hidden rounded-3xl border border-gray-200/80 bg-white/80 shadow-[0_18px_55px_-40px_rgba(15,23,42,0.45)] dark:border-gray-700/80 dark:bg-gray-900/55"
        >
            <div
                class="border-b border-gray-200/80 bg-gradient-to-r from-gray-100/90 to-gray-50/80 px-4 py-3 dark:border-gray-700/80 dark:from-gray-800/90 dark:to-gray-900/70"
            >
                <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div class="min-w-0">
                        ...
                        <div class="mt-1 break-all text-sm font-semibold text-gray-900 dark:text-gray-100">
                            {{ config2?.name || "config" }} <span class="text-gray-400">→</span>
                            {{ config1?.name || "config" }}
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2 text-xs font-medium">
                        <span
                            class="inline-flex items-center gap-2 rounded-full border border-emerald-200/80 bg-emerald-50 px-3 py-1 text-emerald-700 dark:border-emerald-900/70 dark:bg-emerald-500/10 dark:text-emerald-200"
                        >
                            <span class="inline-flex h-2 w-2 rounded-full bg-emerald-500"></span>
                            +{{ addedLinesCount }}
                        </span>
                        <span
                            class="inline-flex items-center gap-2 rounded-full border border-red-200/80 bg-red-50 px-3 py-1 text-red-700 dark:border-red-900/70 dark:bg-red-500/10 dark:text-red-200"
                        >
            ...
            <div
                class="grid grid-cols-2 border-b border-[#d0d7de] bg-[#f6f8fa] text-xs font-semibold uppercase tracking-[0.24em] text-gray-500 dark:border-[#30363d] dark:bg-[#161b22] dark:text-gray-400"
            >
                <div class="flex items-center justify-between px-4 py-3">
                    <span>Старая версия</span>
                    <span class="font-mono normal-case tracking-normal text-gray-400 dark:text-gray-500">{{
                        config2?.modTime
                    }}</span>
                </div>
                <div
                    class="flex items-center justify-between border-l border-[#d0d7de] px-4 py-3 dark:border-[#30363d]"
                >
            ...
            <div class="max-h-[70vh] overflow-auto bg-[#f6f8fa] dark:bg-[#0d1117]">
                ...
                >
                    <div
                        :class="[
                            'min-w-0 border-r border-[#d0d7de] dark:border-[#30363d]',
                            sideClasses(row.left?.type),
                        ]"
                    >
                        <div class="flex min-w-0 items-center">
                            <div
                                class="flex w-14 shrink-0 items-start justify-end border-r border-black/5 px-2 py-1.5 font-mono text-[11px] text-gray-400 dark:border-white/5 dark:text-gray-500"
                            >
                                {{ row.left?.lineNumber ?? "" }}
                            </div>
                            <div
                                class="flex shrink-0 items-start justify-center px-1 py-1.5 font-mono text-[12px] font-semibold"
                            >
                                {{ changeMarker(row.left?.type) }}
                            </div>
                            <pre class="min-w-0 flex-1 overflow-x-auto px-2 py-1.5 font-mono text-[12px] leading-6">{{
                                displayLine(row.left?.text)
                            }}</pre>
                        </div>
                    </div>
...
export default defineComponent({
    ...
    methods: {
                        ...
                        if (nextPart?.added) {
                            ...
                            for (let index = 0; index < maxLength; index++) {
                                rows.push({
                                    id: `row-${rowIndex++}`,
                                    left:
                                        removedLines[index] !== undefined
                                            ? { text: removedLines[index], lineNumber: oldLine++, type: "removed" }
                                            : null,
                                    right:
                                        addedLines[index] !== undefined
                                            ? { text: addedLines[index], lineNumber: newLine++, type: "added" }
                                            : null,
                                });
                            }
                            i++;
        ...
        sideClasses(type?: "added" | "removed" | "same"): string {
            if (type === "added")
                return "bg-[#dafbe1] text-[#1f2328] dark:bg-[rgba(46,160,67,0.15)] dark:text-[#e6edf3]";
            if (type === "removed")
                return "bg-[#ffebe9] text-[#1f2328] dark:bg-[rgba(248,81,73,0.15)] dark:text-[#e6edf3]";
