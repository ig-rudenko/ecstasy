...
export default defineComponent({
        ...
        summaryCards() {
            ...
            return [
                ...
                },
                {
                    ...
                    toneClass:
                        "border-slate-200/80 bg-slate-50/80 text-slate-800 dark:border-slate-700/80 dark:bg-slate-800/70 dark:text-slate-100",
                },
            ];
        },
    },
    methods: {
        async startDiagnostic() {
            ...
            try {
                const response = await api.get(`/api/v1/devices/${this.deviceName}/cable-diag?port=${this.port}`);
...
<template>
    <div class="flex flex-col gap-4">
        ...
        <div
            ...
            class="flex items-center justify-center rounded-3xl border border-dashed border-sky-200/80 bg-sky-50/60 px-4 py-10 dark:border-sky-900/70 dark:bg-sky-950/20"
        >
        ...
        <div v-else-if="diagInfo" class="flex flex-col gap-4">
            <section v-if="isCopperPort" class="grid gap-4 xl:grid-cols-5">
                <div
                    class="overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white/80 xl:col-span-5 dark:border-slate-700/80 dark:bg-slate-950/35"
                >
                    <div class="border-b border-slate-200/80 px-4 py-3 dark:border-slate-700/80">
                        ...
                        <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                            Статус пар и ориентировочная длина обрыва или замыкания.
                        </div>
                    </div>
            ...
            >
                <div class="border-b border-slate-200/80 px-4 py-3 dark:border-slate-700/80">
                    ...
                    <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        Текущие значения модуля и предупреждающие пороги.
                    </div>
                </div>
                ...
                <DataTable
                    ...
                    :pt="{
                        ...
                        column: {
                            headerCell: {
                                class: 'border-b border-slate-200/80 bg-slate-50/80 px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:border-slate-700/80 dark:bg-slate-900/80 dark:text-slate-400',
                            },
                            bodyCell: {
                                class: 'border-b border-slate-200/70 px-4 py-3 align-top text-slate-700 dark:border-slate-800/80 dark:text-slate-200',
                            },
                        },
                        pcPaginator: {
                            root: {
                                class: 'border-t border-slate-200/80 bg-white/70 px-2 py-2 dark:border-slate-700/80 dark:bg-slate-900/60',
                            },
                        },
                    }"
                >
