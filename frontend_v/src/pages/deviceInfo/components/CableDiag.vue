        ...
        <div v-if="diagInfo.len && !diagInfo.sfp">
          ...
          <div style="text-align: right;">
            <div v-if="diagInfo.pair1">
              ...
              <img title="open" style="vertical-align: middle; margin: 0 3px 0 10px; height: 40px" :src="'/img/rj45-status-' + diagInfo.pair1.status + '-left.png'">
            </div>

            <div v-if="diagInfo.pair2">
              Пара 2 - {{ diagInfo.pair2.len }} м.
              <img title="open" style="vertical-align: middle; margin-left: 12px; height: 40px" :src="'/img/rj45-status-' + diagInfo.pair2.status + '-right.png'">
            </div>
          </div>
        </div>
    ...
    sfpData() {
      if (this.diagInfo && this.diagInfo.sfp) {
        return Object.keys(this.diagInfo.sfp).map((key) => {
          const sfpItem = this.diagInfo.sfp[key];
          return {
            parameter: key,
            current: sfpItem.Current,
            low_warning: sfpItem["Low Warning"],
            high_warning: sfpItem["High Warning"],
            status: sfpItem.Status
          };
        });
      }
      return [];
