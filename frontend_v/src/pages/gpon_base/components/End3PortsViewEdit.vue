    ...
    <div v-for="port in end3PortsArray" class="items-center flex flex-row gap-10 py-1">
        ...
        <div>
            <Select
                ...
                v-model="port.status"
                :options="['empty', 'active', 'pause', 'reserved', 'bad']"
        ...
        <div>
            <div class="flex items-center gap-3" v-for="subscriber in port.subscribers">
                <router-link
                    v-if="hasPermissionsToViewSubscriber"
            ...
            <div class="text-muted items-center flex gap-3" v-if="!port.subscribers.length">
                ...
                <template v-if="hasPermissionsToCreateSubscriberData">
                    <Button
                        text
                        size="small"
                        v-tooltip="'Добавить абонента'"
                        severity="success"
                        @click="
                            showCreateSubscriberDataDialog[port.number] = !showCreateSubscriberDataDialog[port.number]
                        "
                    >
                        <svg
                    ...
                    <Dialog
                        v-model:visible="showCreateSubscriberDataDialog[port.number]"
                        class="w-full h-full"
                        modal
...
export default {
    ...
    computed: {
        ...
        hasPermissionsToViewSubscriber() {
            return ["gpon.view_customer", "gpon.view_subscriberconnection"].every((elem) => {
                return this.userPermissions.includes(elem);
            });
        },
