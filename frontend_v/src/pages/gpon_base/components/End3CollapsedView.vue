<template>
    ...
    <div>
        <template v-for="(line, index) in customerLines">
            ...
            >
                ...
                <div>
                    <Button
                        v-if="line.detailInfo || line.capability?.length"
                        @click="$emit('deleteInfo', index)"
                        severity="warn"
                        outlined
                        rounded
                        size="small"
                ...
                <div v-if="editMode && hasPermissionToDeleteEnd3">
                    <Button
                        v-if="line.detailInfo || line.capability?.length"
                        @click="deleteEnd3($event, line, index)"
                        rounded
                        size="small"
                        severity="danger"
                        icon="pi pi-trash"
                ...
                <div v-if="line.errorStatus" class="alert alert-danger">
                    Ошибка при загрузке данных. <br />
                    {{ line.errorMessage || "" }} <br />
                    Статус: {{ line.errorStatus }}
                </div>
            </div>
    ...
    <div v-if="showAddButton" class="py-2">
        ...
        <Dialog v-model:visible="showAddNewEnd3Dialog" modal header="Добавление end3" class="max-w-200 w-full">
            <div>
                <SplittersRizersFind
                    :init="newEnd3.existingSplitter"
                    :type="newEnd3.type"
                    @change="
                        (e) => {
                            newEnd3.existingSplitter = e.value;
                        }
                    "
                />
