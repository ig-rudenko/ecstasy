<template>
    <div class="flex items-center py-3">
        <!-- Сохранить изменения -->
        <Button
            v-if="editMode && (hasPermissionToUpdateHouseB || hasPermissionToUpdateHouseOLTState)"
            text
            severity="success"
            @click="updateBuildingInfo"
            class="save-button"
        >
    ...
    <div class="md:ml-10">
            ...
            <template v-else>
                ...
                <div>
                    <template v-if="buildingData.address.building_type === 'building'">
                        Многоквартирный дом. Количество этажей: {{ buildingData.address.floors }} / Количество
                        подъездов: {{ buildingData.address.total_entrances }}
                    </template>
        ...
        >
            ...
            <div v-if="editMode && hasPermissionToUpdateHouseOLTState">
                <InputText
                    v-model.trim="buildingData.entrances"
                    fluid
        ...
        <div class="py-2 flex flex-wrap items-center justify-around sm:grid grid-cols-2">
            ...
            <Textarea
                v-if="editMode && hasPermissionToUpdateHouseOLTState"
                fluid
                auto-resize
                class="rounded-2xl"
