...
<template>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-10 w-full">
        <div class="flex flex-col gap-6">
                    ...
                    <div class="flex flex-col lg:flex-row gap-8 lg:items-start lg:justify-between">
                        <div class="max-w-2xl">
                            ...
                            <p class="mt-2 text-sm sm:text-base text-gray-600 dark:text-gray-300">
                                Укажите адрес в любом распространённом формате MAC или IPv4. Результаты накапливаются
                                сверху — можно сравнивать несколько запросов подряд.
                            </p>
            ...
            <div
                v-if="!results.length"
                class="rounded-3xl border border-dashed border-gray-200/80 dark:border-gray-700/60 bg-white/40 dark:bg-gray-900/20 backdrop-blur px-6 py-10 text-center"
            >
                <div class="text-gray-500 dark:text-gray-400 text-sm">
                    Пока нет запросов. Введите MAC или IP в блоке выше — сюда появятся разворачиваемые карточки с
                    деталями.
                </div>
            </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    >
                                        <div
                                            v-if="info?.device?.name && info.device.name.startsWith('BRAS')"
                                        ...
                                        <div class="grid gap-2 sm:grid-cols-2">
                                            <div>
                                                <span class="text-gray-500 dark:text-gray-400">IP</span> — {{ row.ip }}
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                <span class="text-gray-500 dark:text-gray-400">VLAN</span> —
                                                {{ row.vlan }}
                                            </div>
                                            <div v-if="row.device_name">
                                                <span class="text-gray-500 dark:text-gray-400">Device</span> —
                                                {{ row.device_name }}
                                            </div>
                                            <div v-if="row.port" class="sm:col-span-2">
