...
class TestPermissions(TestCase):
    ...
    def setUpTestData(cls):
        cls.superuser: User = User.objects.create_superuser(
            "ring_manager.TestPermissions-superuser", "<EMAIL>"
        )
        cls.user: User = User.objects.create_user(
            "ring_manager.TestPermissions-user", "<EMAIL>"
        )
        ...
        cls.transport_ring_api_urls = [
            reverse(
                "ring-manager:api:transport-ring-detail", args=(transport_ring.name,)
            ),
            reverse(
                "ring-manager:api:transport-ring-status", args=(transport_ring.name,)
            ),
            reverse(
                "ring-manager:api:transport-ring-solutions", args=(transport_ring.name,)
            ),
            reverse(
                "ring-manager:api:transport-ring-solutions-last",
                args=(transport_ring.name,),
            ),
        ...
        cls.transport_ring: TransportRing = transport_ring
    ...
    def test_common_user_permission(self):
        ...
        self.assertEqual(
            self.client.get(reverse("ring-manager:transport-rings")).status_code, 403
        )
        self.assertEqual(
            self.client.get(reverse("ring-manager:access-rings")).status_code, 403
        )
    ...
    def test_common_user_permission_api(self):
        self.client.force_login(self.user)
        for url in self.transport_ring_api_urls + self.access_ring_api_urls:
            self.assertEqual(self.client.get(url).status_code, 403, url)

    def test_api_with_access_ring_user_permission(self):
    ...
    def test_api_with_access_transport_ring_user_permission(self):
        self.user.user_permissions.add(
            Permission.objects.get(codename="access_transport_rings")
        )
    ...
    def test_superuser_permission_api(self):
        self.client.force_login(self.superuser)
        for url in self.transport_ring_api_urls + self.access_ring_api_urls:
            self.assertNotEqual(self.client.get(url).status_code, 403, url)
