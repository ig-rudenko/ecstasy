<template>
  <button v-tooltip.bottom="'Карты узла сети'" @click="toggleOverlay" class="btn" style="color: #f200ff;">
  ...
  <OverlayPanel ref="zabbixMapsOverlayPanel">

    <div>
      <div class="border-bottom mb-3">Карты Zabbix где имеется данное оборудование</div>
      <div v-for="map in mapsData">
        <a :href="getMapLink(map.sysmapid)" target="_blank" class="text-decoration-none">{{ map.name }}</a>
      </div>
    </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    toggleOverlay(event: Event) {
      (<OverlayPanel>this.$refs.zabbixMapsOverlayPanel).toggle(event);
    }
  }
