...
class Solutions:
    ...
    def _change_port(self, device: Devices, port: str, status: Literal["up", "down"], message: str):
    ...
    def port_set_up(self, device: Devices, port: str, message: str):
        """
        Переводит указанный порт устройства в состояние `admin up`.
        """
        self._change_port(device, port, "up", message)

    def port_set_down(self, device: Devices, port: str, message: str):
        """
    ...
    def delete_vlans(self, vlans: tuple[int, ...], device: Devices, port: str, message: str):
        """
        Удаляет указанные VLAN на порту данного устройства.
        """
        self._change_vlans("delete", vlans, device, port, message)

    def add_vlans(self, vlans: tuple[int, ...], device: Devices, port: str, message: str):
        """
...
class SolutionsPerformer:
    ...
    @staticmethod
    def _get_device(device: dict[str, str]) -> Devices:
        """
        ...
        try:
            ...
            return Devices.objects.select_related("auth_group").get(ip=device["ip"], name=device["name"])
    ...
    def _validate_data_for_vlan_performer(status: str, vlans: Sequence):
        ...
        for vlan in vlans:
            if not isinstance(vlan, int) or vlan < 1 or vlan > 4096:
                raise SolutionsPerformerError(
