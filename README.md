...
- `Cisco`
    - Проверено для: `Nexus 5548UP` `WS-C4500*` `WS-C35*0` `WS-C3750*` `ME-3*00X` `ME-4924` `ME-3400`
- `Eltex`
...
```shell
git clone https://github.com/ig-rudenko/ecstasy
```

Для выбора стабильной версии желательно переходить по тегам,
смотрите [релизы](https://github.com/ig-rudenko/ecstasy/releases). Тег указывается без пробельных символов.

```shell
git checkout v20.0.4
```
...
Импорт активных узлов сети из Zabbix группы `devs` в новую группу `devs` ecstasy, с указанием группы авторизации
`creds-1`, а также использование `telnet`
...
Импорт активных узлов сети из диапазона `172.30.0.0/24` Zabbix группы `devs` в новую группу `devs` ecstasy, с указанием
группы авторизации `creds-1`, а также использование `ssh` (По умолчанию SSH)
как способа подключения к оборудованию:
